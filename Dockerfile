...
# --no-deamon: docker 이미지를 빌드하는 컨테이너는 일회성이므로 메모리 누수 위험이 있는 gradle deamon 프로세스를 켜지 않도록 설정
COPY gradlew settings.gradle.build.gradle ./
COPY gradle ./gradle
RUN chmod +x gradle && ./gradle dependencies --no-daemon
...
# 네트워크 요청 도구인 curl 설치(리눅스의 postman), 불필요한 패키자 설치를 막고 패키지 목록 임시 파일들을 전부다 삭제해서 이미지 용량을 줄이자
# curl 설치 이유: health 체크용
RUN apt-get update && apt-get install -y --no-install-recommends curl \
    && rm -rf /var/lib/apt/lists/*

# build 스테이지에서 만든 jar를 카피해서 app.jar 이름으로 붙여넣기
COPY --from=build /workspace/build/libs/sprintlog-boot-0.0.1-SNAPSHOP.jar app.jar
...
# EXPOSE는 문서 키워드, 실제로 노출시키려면 컨테이너 생성 시에 -p를 사용해서 노출시켜야 한다.
EXPOSE 8080 9090
...
# CMD, RUN: 기본 실행 명령을 의미, 컨테이너 실행 시에 다른 명령어가 주어지면 그 명령어도 대체됨
# ENTRYPOINT: 반드시 실행되어야 할 명령어 의미, 다른 명령어로 대체되지 않음
