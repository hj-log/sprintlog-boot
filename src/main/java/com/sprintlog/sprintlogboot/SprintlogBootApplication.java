...
import com.sprintlog.sprintlogboot.lifecycle.*;
...
public class SprintlogBootApplication {
    ...
    // 내가 직접 만든 클래스는 직접 @component 등을 사용해서 Bean 등록하면 되지만,
    // 라이브러리 등에서 제공하는 객체는 메서드 안에서 객체를 생성한 후에 리턴되는 객체를 Bean으로 등록하기 위해 @Bean을 사용.
    // CommandLineRunner: Spring Boot가 구동이 완료되고, 딱 한번 실행되는 코드르르 정의할 때 사용하는 인터페이스.
