...
public class ActivityByTypeVaildator implements ConstraintValidator<ValidActivityByType, CreateActivityRequest> {

    @Override
    public boolean isValid(CreateActivityRequest req, ConstraintValidatorContext context) {

        // type 자체가 null인 경우는 @NotNull이 잡는다(여기서는 통과)
    ...
    private boolean requiredField(ConstraintValidatorContext ctx, boolean missing, String field, String message ) {
        ...
        ctx.disableDefaultConstraintViolation(); // 기본 메세지 비활성화
        ctx.buildConstraintViolationWithTemplate(message) // 우리 메세지를 세팅
                .addPropertyNode(field) // 메세지를 필드에 달아준다.
